...
func (l GormLogger) Info(ctx context.Context, str string, args ...interface{}) {
	l.logger().Sugar().Debug(str, args)
}

// Warn implements logger.Interface
func (l GormLogger) Warn(ctx context.Context, str string, args ...interface{}) {
	l.logger().Sugar().Warn(str, args)
}

// Error implements logger.Interface
func (l GormLogger) Error(ctx context.Context, str string, args ...interface{}) {
	l.logger().Sugar().Error(str, args)
}
...
func (l GormLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	if err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			l.logger().Warn("Database ErrRecordNotFound", logFiles...)
		} else {
			// 其他错误使用 error 等级
			logFiles = append(logFiles, zap.Error(err))
			l.logger().Error("Database Error", logFiles...)
		}
	}

	// 慢查询日志
	if l.SlowThreshold != 0 && elapsed > l.SlowThreshold {
		l.logger().Warn("Database Slow Log", logFiles...)
	}

	// 记录所有的SQL请求
	l.logger().Debug("Database Query", logFiles...)
}
